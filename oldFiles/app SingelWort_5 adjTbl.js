...
function othrTbls(){
let allContent= document.querySelectorAll('div>div>section>header')
...
function getSatze(){
let allContent= document.querySelectorAll('div>div>section>header>h2')
allContent.forEach((itm)=>{
 ...
 let zBel = document.querySelectorAll(".vBsp>ul");
...
function timeOutGapi(callback, d = false ){ 
    ...
    if(!d || _gapi_F){
        ...
        if(loopCount<20){
            loopCount++
            timeoutId = setTimeout(()=>{timeOutGapi(getWort,true)}, 100);
        }else{
            timeOutGapi(getWort,false)
        }
    }
}
